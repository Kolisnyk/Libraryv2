...
#INSERT INTO "main"."Draftlibrary" ("genre","title","author","cover","content","created_at","authorized_name") VALUES (?1,?2,?3,?4,?5,?6,?7)

class Draftbook

  @@SQLITE_DB_FILE = 'Draftlibrary.sqlite'

  def self.post_types
    {'Fantasy_' => Fantasy_, 'Detective_' => Detective_, 'Humour_' => Humour_}
  end
  ...
  def self.find(limit, genre, id)
    ...
    if !id.nil?
      ...
      query = "SELECT rowid, * FROM Draftlibrary "
  ...
  def to_strings
    time_string = "The book included in the Draftlibrary: #{@created_at.strftime("%Y,%m,%d, %H:%M:%S")} \n\r \n\r"
